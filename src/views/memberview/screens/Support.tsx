        ...
        <div className="relative z-10 flex flex-col items-center gap-6">
          ...
          <div className="relative w-full max-w-2xl">
            ...
            <input
              ...
              className="w-full pl-14 pr-6 py-4 rounded-2xl bg-white border-none shadow-xl focus:ring-4 focus:ring-white/20 text-lg outline-none"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <div className="flex flex-wrap justify-center gap-3">
            {['Refund', 'Membership', 'Loss Card', 'QR Code'].map(tag => (
              <button key={tag} className="px-4 py-1.5 bg-white/10 text-white rounded-full text-sm font-medium hover:bg-white/20 transition-all">
                {tag}
              </button>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold text-slate-800">Frequent Questions</h3>
            <button className="text-primary text-sm font-bold flex items-center gap-1 hover:underline">
              View all <HelpCircle size={14} />
            </button>
          </div>
            ...
            {faqs.map((faq, index) => (
              ...
              >
                ...
                >
                  ...
                  {openFaq === index ? <ChevronUp size={20} className="text-primary" /> : <ChevronDown size={20} className="text-slate-400" />}
                </button>
                <AnimatePresence>
                  {openFaq === index && (
                    <motion.div
          ...
          <div className="mt-4 bg-white border border-slate-100 p-8 rounded-3xl shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select className="bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-primary/20 transition-all text-sm appearance-none">
                  <option>Select vehicle...</option>
                  <option>51A-123.45 (Honda Winner)</option>
                  <option>51B-987.65 (Toyota Vios)</option>
